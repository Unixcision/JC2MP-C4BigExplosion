...
function C4:GetCellId() -- Will always be the same as the parent WNO
	...
	if Server then
		...
		if self:GetPosition().y < 200 then
			ClientEffect.Play(AssetLocation.Game, {
                ...
                local positio = self:GetPosition()
                if sender:GetPosition():Distance(positio) < 40 then
		local falloff = (40 - sender:GetPosition():Distance(positio)) / (40 * 0.6)
		...
		if sender:InVehicle() then
			...
			sender:GetVehicle():SetLinearVelocity(sender:GetVehicle():GetLinearVelocity() + ((sender:GetVehicle():GetPosition() - positio):Normalized() * 40 * 2 * falloff))
		end
